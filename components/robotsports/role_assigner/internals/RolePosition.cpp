...
MRA::Geometry::Point RolePosition::determineDynamicRolePosition(defend_info_t& rDefend_info, planner_target_e& planner_target, int& r_gridFileNumber, role_e role,
    ...
    else if ((role == role_ATTACKER_MAIN) and r_role_assigner_data.ball.status != OWNED_BY_PLAYER) { // setplay handled before there
        // dr_INTERCEPTOR
    ...
    else if (role == role_ATTACKER_MAIN and r_role_assigner_data.ball.status == OWNED_BY_PLAYER) {
        //    case dr_BALLPLAYER: {
...
MRA::Geometry::Point RolePosition::calculateSetPlayReceiverPosition(const RoleAssignerData& r_role_assigner_data)
{
    ...
    for (unsigned int idx = 0; idx < r_role_assigner_data.team.size(); idx++) {
        if (r_role_assigner_data.previous_results[idx].present and r_role_assigner_data.previous_results[idx].role == role_ATTACKER_ASSIST) {
            MRA::Geometry::Point previousEndPos = MRA::Geometry::Point(r_role_assigner_data.previous_results[idx].end_position.x, r_role_assigner_data.previous_results[idx].end_position.y);
