...
class Builder():
    ...
    def run_pre_test(self) -> None:
        ...
        cmd = 'rm -rf /tmp/mra_logging_testsuites'
        self.run_cmd(cmd)
        # set test configuration (maybe we need some scripting for this ... ?)
        cmd = 'echo \'{"folder":"/tmp/mra_logging_testsuites","filename":"\u003cmaincomponent\u003e_\u003cpid\u003e.spdlog","general":{"component":"MRA","level":"TRACE","enabled":true,"dumpTicks":true,"maxLineSize":1000,"maxFileSizeMB":10,"pattern":"[%Y-%m-%dT%H:%M:%S.%f] [%P/%t/%k] [%^%l%$] [%s:%#,%!] %v"}}\' > ' + TESTSUITE_SHM_FILE
        self.run_cmd(cmd)
