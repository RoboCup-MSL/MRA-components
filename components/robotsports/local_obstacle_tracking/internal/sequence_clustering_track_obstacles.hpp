...
typedef struct tag_hypothesis_w {
    ...
    int association_id;     /* identifier of the Kalman filter obstacle that is associated with the current measurement */
    int filter_id[MAXNOBJ_GLOBAL];  /* array of Kalman filter identifiers, representing the detected obstacles according to this hypothesis */
    unsigned number_of_obstacles;               /* number of detected obstacles */
...
typedef struct tag_kalman_filter_w {
    ...
    int label;              /* obstacle label, 0 = unknown obstacle (default), 1..7 = turtle, 8.. = opponent */
} kalman_filter_w;
...
typedef struct tag_scw_parameters {
    ...
    unsigned maxnobj;            /* maximum number of obstacles in an hypothesis */
    double clipradius;      /* clipping radius relative to own position */
    double kscale;          /* scale factor for Kalman gain */
    double maxage;          /* maximum age for non-updated obstacles */
    double labelbound;      /* minimum required likelihood for obstacle association */
} scw_parameters;
