                                        ...
                                        MRA::RobotsportsLocalBallTracking::OutputType &output)
{
    ...
    if (delta_t > 0.0) {
        ...
        //TODO use last sample time value as thesample time value, knowing we do have jitter
        xy_t speed = { 0, 0 };
        int ret = calculate_velocity(speed, ball_position_history, params.ball_vel_filter_order(), 1/params.sample_frequency());
        if (ret == 0) {
