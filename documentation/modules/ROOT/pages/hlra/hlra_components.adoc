= Humanoid League Reference Architecture Components

== C4 model

The link:https://www.c4model.com[C4 model] is used do described the Humanoid League reference architecture.
...
* ROS2 will be used as framwork for the Humanoid League Reference Architecture. ROS2 is a middleware suite, which is used by many robots, include commercial available humanoid robots.
* Components are published as ROS2 components. ROS2 components can be combined in a single process with the lower overhead and optionally more efficient communication.  +
(source: link:https://docs.ros.org/en/humble/Concepts/Intermediate/About-Composition.html[ROS2 Composition]
and link:https://docs.ros.org/en/humble/Tutorials/Demos/Intra-Process-Communication.html)[Intra Process Communication]).
* An interface for multiple programming languages is preferred:
	** C++ (high performance)
...
include::hlra_standard_data_definitions.adoc[]
...
Each HLRA component will describe the runtime frequency where it was designed for.
...
The contents of the attributes varies per HLRA component.
Each HLRA component variant must have the same inputs and outputs. This is an essential requirement to be able to replace an HLRA component variant by another variant. The parameter can vary per variant.
...
This can be achieved by using ROS2 messages.

REQ-DDF-01::
The Data definition format of the Humanoid League Reference architecture must support serialization. Serialization is the process of translating a data structure or object state into a format that can be stored or transmitted +
*Rationale*  Serialization allow Logging and store the data in a database, also usefull for testing purposes.

* rclcpp::Serialization Class Reference: https://docs.ros.org/en/humble/p/rclcpp/generated/classrclcpp_1_1Serialization.html

* rclcpp::SerializedMessage Class Reference: https://docs.ros.org/en/humble/p/rclcpp/generated/classrclcpp_1_1SerializedMessage.html
...
== Humanoid League Reference Architecture Repository
...
The Humanoid League Reference Architecture Repository will contain the basic  definitions for the standard. For each component as defined by the Humanoid League Reference Architecture there will repository available.
Within the specific HLRA component repository multiple variants can be published. Each variant must be self-contained.

REQ-REP-01::
There must be a central repository for HLRA software datatypes.
*Rationale*: A central repository cand to ensure consistency, correctness and provide the software datatype to teams.

DEC-REP-01:: #TODO
The Humanoid League Reference architecture will be hosted on a public repository within the link:https://github.com/RoboCup-MSL[RoboCup-MSL repository] on GitHub. The datatypes will be defined in: HLRA-datatypes. +
...
From an HLRA component multiple variants can exist. A variant created by Team X and a variant created by Team Y. For a variant a multiple versions can exists. +
Eg.: HLRA/GetBall/v1/Falcons_v1
