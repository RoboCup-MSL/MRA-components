...
//        	logAlways("feature %d: t=%f, x=%f, y=%f, z=%f, sigma=%f",
//                        i, phyp->fbuf.timestamp[i], phyp->fbuf.x[i], phyp->fbuf.y[i], phyp->fbuf.z[i], phyp->fbuf.sigma[i]);
//        }
        return BM_SUCCESS;
}
...
static int fbuf_cleanup(phypothesis phyp)
{
        ...
        if (idx1 < 0) { idx1 = MAXFEATBUF - 1; }
        
        idx2 = idx1 - 1;
        if (idx2 < 0) { idx2 = MAXFEATBUF - 1; }
...
static double ma_get(phypothesis phyp)
{
        ...
        idx1 = phyp->ma_idx - 1;
        if (idx1 < 0) { idx1 += MA_N + 1; }

        idx2 = idx1 - MA_N;
        if (idx2 < 0) { idx2 += MA_N + 1; }
...
static int fit_line_xy(double* theta, double* val, double* timestamp, double* sigma, int* idx, int n, double* exp_time, int* isFree, sc_global_data * pscgd)
{
        int i;
        double a, b, c, d, e, f, w, det, t;

        /* too few data points */
        if (n < MIN_NUMBER_OF_FEAT) { return BM_FIT_ERROR; }
...
//         logAlways("fit_line_xy: using history of %d features in regression.", i);
...
static int fit_curve_z(double* theta, double* val, double* timestamp, double* sigma, int* idx, int n, double* exp_time, int* isFree, sc_global_data * pscgd)
{
        int i;
        double a, b, c, d, e, f, w, det, t, t2, v;

        /* too few data points */
        if (n < MIN_NUMBER_OF_FEAT) { return BM_FIT_ERROR; } /* 1 is too few, set to 4 to ignore few-points-estimate */
        ...
        //logAlways("fit_curve_z: using history of %d features in regression.", i);
...
// TODO
//	logAlways("Hypothesis: %d", i);
//        switch (phyp->nobj) {
//        case 0:
//        	logAlways("   No ball.");
//                break;
//        case 1:
//        	logAlways("   Uid = %d", phyp->obs.uid);
//        	logAlways("   Ball at (%f, %f, %f)", phyp->obs.xh[0], phyp->obs.xh[2], phyp->obs.xh[4]);
//        	logAlways("   Type = %d", phyp->obs.label);
//        	logAlways("   Tupd = %f", phyp->obs.tupd);
//                if (phyp->obs.dupd == BU_TODO ) {
//                	logAlways("   Ball will be updated.");
//                }
//                logAlways("   Association flag = %d.", phyp->association_flag);
//                logAlways("   Time = %f", phyp->obs.time);
//                break;
//        }
//        logAlways("   Probability = %f", phyp->p);
//        logAlways("   Confidence  = %f", phyp->mavg);
//
//        //getchar();  // pause
//
        return BM_SUCCESS;
...
static int associate_with_existing_ball(int i, int j, ball_feature_t * pbfeat, sc_global_data * pscgd)
{
        ...
        if (j >= MAXHYP ) { return BM_ERROR_MAXHYP; }
...
#ifdef BMDEBUG
        	logAlways("hyp %d: nobj = %d   p = %f", i, pscgd->hyp[i].nobj, pscgd->hyp[i].p);
#endif
...
//                        mexPrintf("nbuf van hyp %d = %d\n", i, pscgd->hyp[i].nfbuf);
                        ...
                        case 1:
                                ...
                                break;
                        default:
                                if (fit_line_xy(theta, pscgd->hyp[i].fbuf.x, pscgd->hyp[i].fbuf.timestamp, pscgd->hyp[i].fbuf.sigma, idx, 
                                                pscgd->hyp[i].nfbuf, pscgd->hyp[i].fbuf.exp_time, pscgd->hyp[i].fbuf.isFree, pscgd) == BM_SUCCESS ) {
                                        pscgd->hyp[i].obs.xh[0] = theta[1];
                                        pscgd->hyp[i].obs.xh[1] = theta[0];
//                                        mexPrintf("hyp %d vx = %f\n", i, theta[0]);
                                } else {
//                                          logAlways("observer_update: fit_line_xy failed (x).");
                                        pscgd->hyp[i].obs.xh[0] = pscgd->hyp[i].fbuf.x[idx[0]];
                                        pscgd->hyp[i].obs.xh[1] = 0.0;
                                }

                                if (fit_line_xy(theta, pscgd->hyp[i].fbuf.y, pscgd->hyp[i].fbuf.timestamp, pscgd->hyp[i].fbuf.sigma, idx, 
                                                pscgd->hyp[i].nfbuf, pscgd->hyp[i].fbuf.exp_time, pscgd->hyp[i].fbuf.isFree, pscgd) == BM_SUCCESS) {
                                        pscgd->hyp[i].obs.xh[2] = theta[1];
                                        pscgd->hyp[i].obs.xh[3] = theta[0];
//                                        mexPrintf("hyp %d vy = %f\n", i, theta[0]);
                                } else {
//                                          logAlways("observer_update: fit_line_xy failed (y).");
                                        pscgd->hyp[i].obs.xh[2] = pscgd->hyp[i].fbuf.y[idx[0]];
                                        pscgd->hyp[i].obs.xh[3] = 0.;
                                }

                                if (fit_curve_z(theta, pscgd->hyp[i].fbuf.z, pscgd->hyp[i].fbuf.timestamp, pscgd->hyp[i].fbuf.sigma, idx, 
                                        pscgd->hyp[i].nfbuf, pscgd->hyp[i].fbuf.exp_time, pscgd->hyp[i].fbuf.isFree, pscgd) == BM_SUCCESS) {
                                        pscgd->hyp[i].obs.xh[4] = theta[1];
                                        pscgd->hyp[i].obs.xh[5] = theta[0];
                                } else {
                                        //logAlways("observer_update: fit_curve_z failed (z).");
                                        pscgd->hyp[i].obs.xh[4] = pscgd->hyp[i].fbuf.z[idx[0]];
                                        pscgd->hyp[i].obs.xh[5] = 0.;
                                }
                                break;
...
#ifdef BMDEBUG
                        logAlways("tupd = %f, time = %f", pscgd->hyp[i].obs.tupd, pscgd->hyp[i].obs.time);
#endif
                        /* ball time becomes update time */
                        pscgd->hyp[i].obs.time = pscgd->hyp[i].obs.tupd;
                } else {
#ifdef BMDEBUG
                	logAlways("hyp %d not updated.", i);
#endif
                }
        }
        return BM_SUCCESS;
...
static int likelihood_update(ball_feature_t * pbfeat, sc_global_data * pscgd)
{
        ...
        for (j = 0; j < pscgd->nhyp; j++) {
                ...
                if ( (pscgd->nhyp > 1) && ( (j + 1) % 3 > 0) ) {
                        ...
                        p = exp( -0.5 * ( (tttx * tttx) / (sigmax * sigmax) +
                                          (ttty * ttty) / (sigmay * sigmay) +
                                          (tttz * tttz) / (sigmaz * sigmaz) ) );
                        //logAlways("  Existing ball hypothesis %d: p = %40.33f", j, p);
                        if (p > pmax) { pmax = p; }
                }
        }
        pawn = 1. - pmax;
        ...
        for (i = 0; i < pscgd->nhyp; i++) {
                ...
                case ASSOCIATE_WITH_BALL:
                        ...
                        p = exp( -0.5 * ( (tttx * tttx) / (sigmax * sigmax) +
                                          ...
                                          (tttz * tttz) / (sigmaz * sigmaz) ) );
#ifdef BMDEBUG
                        logAlways("hyp %d: ASSOCIATE_WITH_BALL: p factor = %f", i, p);
#endif
                        /* reset update flag */
                ...
                case ASSOCIATE_WITH_CLUTTER:
                        ...
                        ttty = pbfeat->y-pscgd->hyp[i].obs.xh[2];
                        tttz = pbfeat->z-pscgd->hyp[i].obs.xh[4];
                        p = 1. - exp( -0.5 * ( (tttx * tttx) / (sigmax * sigmax) +
                                               (ttty * ttty) / (sigmay * sigmay) +
                                               (tttz * tttz) / (sigmaz * sigmaz) ) );
#ifdef BMDEBUG
                        logAlways("hyp %d: ASSOCIATE_WITH_CLUTTER: p factor = %f", i, p);
#endif
                        /* reset update flag */
                ...
                case ASSOCIATE_WITH_NEW:
                        p = pawn;
#ifdef BMDEBUG
                        logAlways("hyp %d: ASSOCIATE_WITH_NEW: p factor = %f", i, p);
#endif
                        /* reset update flag */
                        pscgd->hyp[i].obs.dupd = BU_NONE; 
...
#ifdef BMDEBUG
                logAlways("hyp %d: p = %f", i, pscgd->hyp[i].p);
#endif
        }
...
static int normalization(sc_global_data * pscgd)
{
        ...
        for (i = 0; i < pscgd->nhyp; i++) {
#ifdef BMDEBUG
        	logAlways("norm: hyp[%d]: p = %f", i, pscgd->hyp[i].p);
#endif
                sum += pscgd->hyp[i].p;
        }
        if (sum <= 0.0) { return BM_ERROR_NORM; }
        for (i = 0; i < pscgd->nhyp; i++) {
                pscgd->hyp[i].p /= sum;
        }
        return BM_SUCCESS;
...
#ifdef BMDEBUG
        logAlways("psmall = %f", psmall);
#endif
        ifilter = n - pscgd->par.nkeep;
        if (ifilter < 0) { ifilter = 0; }
    
        pfilter = p[idx[ifilter]];
#ifdef BMDEBUG
        logAlways("pfilter = %f", pfilter);
#endif
        /* apply filter action to the system... */
        j = 0;
        for (i = 0; i < n; i++) {
//#ifdef BMDEBUG
//              logAlways("filter hyp %d: p = %f, uid = %d", idx[n-1-i], pscgd->hyp[idx[n-1-i]].p, pscgd->hyp[idx[n-1-i]].obs.uid);
//#endif
...
//              logAlways("current track_uid = %d at id = %d", pscgd->track_uid, iret);
                ...
                case 1:
                        ...
                        //logAlways("track_uid still belongs to best balls, keep current track_uid.");
                        break;
                }
        }

        //logAlways("track_uid na = %d, p = %f", pscgd->track_uid, buid.p[uid_get_id(pscgd->track_uid, &buid)]);

#ifdef BMDEBUG
        logAlways("new number of hypotheses = %d", j);
#endif
        pscgd->nhyp = j; /* new number of active hypotheses */
...
static int mape(sc_global_data * pscgd)
{
        ...
        if (i_mape < (pscgd->nhyp-1) ) {
                ...
                for (i = i_mape + 1; i < pscgd->nhyp; i++) {
                        if ( (pscgd->hyp[i].obs.uid == pscgd->track_uid) &&
                             (pscgd->hyp[i].p > pscgd->hyp[i_mape].p) ) {
                                i_mape = i;
                        }
                }
        }
...
*/
...
#ifdef BMDEBUG
        //logAlways(" feature %d: x = %f, y = %f, z = %f, conf = %f", i, (pbfeat+i)->x, (pbfeat+i)->y, (pbfeat+i)->z, (pbfeat+i)->conf);
#endif
...
#ifdef BMDEBUG
        logAlways("sc_bm reports: %d valid features found at time %f.", n, time);
        for (i = 0; i < n; i++) {
        	logAlways("valid[%d] = %d, ts[%d] = %f", i, valid[i], i, ts[i]);
        }
#endif
...
#ifdef BMDEBUG
                logAlways("sc_bm reports:");
                logAlways("   time = %f s", time);
                logAlways("   %d measurements at times:", n);
                for (i = 0; i < n; i++) {
                	logAlways("      %f x = %f, y = %f, z = %f, conf = %f", (pbfeat+valid[idx_t[i]])->timestamp, (pbfeat+valid[idx_t[i]])->x, (pbfeat+valid[idx_t[i]])->y, (pbfeat+valid[idx_t[i]])->z, (pbfeat+valid[idx_t[i]])->conf);
                }
#endif
...
#ifdef BMDEBUG
                        logAlways("Processing feature #%d:", i);
                        print_hypotheses(pscgd);
#endif
        ...
        i_mape = mape(pscgd);
//         logAlways("p of winner = %f", pscgd->hyp[i_mape].p);

#ifdef BMDEBUG
        logAlways("Winning hypothesis at t = %f: i_mape = %d, uid = %d, p = %f", time, i_mape, pscgd->hyp[i_mape].obs.uid, pscgd->hyp[i_mape].p);
         switch ( pscgd->hyp[i_mape].nobj ) {
        case 0:
        	logAlways("-> No ball.");
                break;
        case 1:
        	logAlways("-> Ball position: ");
        	logAlways("%f %f %f", pscgd->hyp[i_mape].obs.xh[0], pscgd->hyp[i_mape].obs.xh[2], pscgd->hyp[i_mape].obs.xh[4]);
                break;
         }
#endif
        ...
        //logAlways("Winning hypothesis updated_in_timestep = %d", pball->isUpd);

        pball->label = pscgd->hyp[i_mape].obs.label;

        pball->timestamp = pscgd->hyp[i_mape].obs.time;

        //logAlways("cputime = %d ms", (int) ((get_time() - t1) * 1000) );
