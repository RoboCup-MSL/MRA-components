    ...
    const mra_common_msgs::msg::WorldState& world_state)
{
    ...
    // Fill world state
    vc_input.world_state.robot.position = VelocityControlConvert::convertFromRosPose(world_state.robot.pose);
    vc_input.world_state.robot.velocity = VelocityControlConvert::convertFromRosTwist(world_state.robot.velocity);
    ...
    vc_input.setpoint.position = VelocityControlConvert::convertFromRosPose(safe_target.pose);
    vc_input.setpoint.velocity = VelocityControlConvert::convertFromRosTwist(safe_target.velocity);
    ...
    if (vc_output.success) {
        result_twist = VelocityControlConvert::convertToRosTwist(vc_output.velocity_rcs);
    } else {
    ...
    const mra_common_msgs::msg::WorldState& world_state)
{
    ...
    // TODO: Implement actual shooting logic when ShootPlanning component is available
    // This should analyze the shoot_target and world_state to determine:
    // - Shooting power based on distance to target
    // - Shooting angle/direction
    // - Whether conditions are suitable for shooting
