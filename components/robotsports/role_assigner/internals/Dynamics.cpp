...
Dynamics::dynamics_t Dynamics::interceptBall(const RoleAssignerBall& rBallObject,
        const Geometry::Point& meCoordinates, double maxSpeed, const Environment& environment, bool move_to_ball_left_field_position) {
    dynamics_t result = {};
    ...
    if (!environment.isInField(result.intercept_position, 0.25*environment.getRobotRadius()))
    {
    ...
    if (result.move_to_ball_leave_field_pos )
    {
        if (!move_to_ball_left_field_position) {
            ...
            result.intercept_position = calculateBallLeavingFieldPoint(rBallObject, environment);
        }
    }
