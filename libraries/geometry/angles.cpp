...
// wrap rotation/angle [rad] to [0, 2pi) for standardized coordinates
...
// wrap rotation/angle [rad] to [-pi, pi) for subsequent abs()-and-compare
double MRA::Geometry::wrap_pi(double rotation_in_radians)
{
    double result = MRA::Geometry::wrap_2pi(rotation_in_radians);
    if (result >= M_PI) result -= 2 * M_PI;
    return result;
}

// relative angle [rad] in MSL-FCS, where 0 is facing opponent goal
