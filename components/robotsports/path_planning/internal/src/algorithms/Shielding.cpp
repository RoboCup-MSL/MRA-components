...
void Shielding::execute(PathPlanningData &data)
{
    ...
    MRA_LOG_DEBUG("closestOpponentDistance=%.2f", closestOpponentDistance);

    // check if opponent closeby
    if (closestOpponentDistance > shieldingObstacleProximity)
    {
        MRA_LOG_DEBUG("no shielding, closest opponent is too far away");
        return;
    }
    if (closestOpponentDistance < 0.1)
    {
        // cannot calculate a good angle ... this might even need to be a warning in worldModel
        MRA_LOG_DEBUG("no shielding, opponent is standing right on top of robot?!");
        return;
    }
    ...
    for (auto opponent: data.obstacles)
    {
        ...
        {
            MRA_LOG_DEBUG("no shielding, there are two nearby opponent on both sides of the path");
            return;
        }
    }
